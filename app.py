...
# Global model loading for performance (prevents I/O bottlenecks on every request)
sc = None
model = None
try:
...
def result():
    ...
    prediction_value = float(Y_pred[0]) if hasattr(Y_pred, '__getitem__') else float(Y_pred)
