    ...
    if request.method == "POST":
        try:
            ...
            if not os.path.exists(SCALER_PATH) or not os.path.exists(MODEL_PATH):
                return (
                    "Error: Model files not found. Please follow instructions in models/README.md to generate them.",
                    500,
                )
            ...
            prediction_value = (
                float(Y_pred[0])
                if hasattr(Y_pred, "__getitem__") and len(Y_pred) > 0
                else float(Y_pred)
            )

            return render_template(
                "result.html", prediction="{:.2f}".format(prediction_value)
            )
